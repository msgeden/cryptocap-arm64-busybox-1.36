...
int main(void)
{
    ...
    if (fpid == 0) {
        ...
        execl("./app-agrant-dcall-dret", "app-agrant-dcall-dret", NULL);
        // If execl fails
        perror("execl");
        exit(EXIT_FAILURE);
        ...
        //fill a cdcapcl register via syscall
        ...
        printf("Lib1: Sent CURRENT_TASK: 0x%lx\n", lib1_cap.SP_EL0);
        printf("Lib1: Sent SP: 0x%lx\n", lib1_cap.TASK_STRUCT);        
        printf("Lib1: Sent PSTATE: 0x%lx\n", lib1_cap.PSTATE);
        printf("Lib1: Sent TPIDR: 0x%lx\n", lib1_cap.TPIDR_EL0);
        printf("Lib1: Sent TPIDRRO: 0x%lx\n", lib1_cap.TPIDRRO_EL0);
