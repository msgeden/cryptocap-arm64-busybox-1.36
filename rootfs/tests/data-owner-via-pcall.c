...
int main() {
    ...
    read(STDIN_FILENO, &ccall_cap, sizeof(pdom));
    
    /////////////////////////////PREPARE TARGET REGISTER FOR CCALL /////////////////////////////#
    printf("Caller-Data Owner: Setting the target register (CLC)\nCLC.PC=0x%lx CLC.SP=0x%lx, CLC.PTBASE=0x%lx, CLC.MAC=0x%lx, CLC.PID=%ld\n", ccall_cap.PC, ccall_cap.SP, ccall_cap.PT, ccall_cap.MAC, ccall_cap.PID);
    ...
    asm volatile(
        "ldr x0, %[var]"      // Load the value of PID value into x0
        :                     // No output operands
        : [var] "m" (ccall_cap.PID)  // Input operand (memory reference to stack_variable)
    ...
    asm volatile(
        ...
        :                     // No output operands
        : [var] "m" (ccall_cap.PC)  // Input operand (memory reference to stack_variable)
        : "x1"                // Clobbers (x1 register is modified)
    );


    //Call to sender-callee process  (pcall)
    asm volatile (
     "mov x8, #466\t\n"
     "svc #0\n" 
    );
